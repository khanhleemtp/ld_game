...
import { 
    PopupWrapper, PopupBannerText, PopupBannerWrapper, 
    PopupContent, PopupTitle, IconWrapper,
    SelectWrapper
} from './styles'
...
export default function Popup({ isOpenPopup, setIsOpenPopup, login }) {
    ...
    useEffect(()=>{
        if(gameState._id !== "")
          history.push(`/room/${gameState._id}`);
      },[gameState._id]);
    ...
    return (
        <>
            ...
            >
                ...
                <PopupTitle>
                    { login ? `Để đăng nhập, chọn một tài khoản bên dưới: `: 
                        `Cài đặt phòng chơi: `
                    }
                </PopupTitle>
                {
