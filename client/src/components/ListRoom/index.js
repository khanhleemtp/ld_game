...
import { ListRoomContainer,
         RoomTitle, RoomContentWrapper, 
         RoomContentContainer , RoomElementContainer,
         RoomElementAmount, RoomElementName, RoomTitleWrapper, RoomTitleText,
         RoomSearchWrapper, RoomSearchInput
         } from './styles'
        ...
        <ListRoomContainer>
            ...
            <RoomContentContainer>
                <RoomContentWrapper>
                {
                    ...
                    {
                        room.rooms.map(room => (
                            <RoomElementContainer key={room._id}
                                onClick={() => joinRoom(room)}
                            >
                                <RoomElementName>{room.name}</RoomElementName>
                                <RoomElementAmount>{room.players && room.players.length ? room.players.length : 0 }/{room.maxPlayer}</RoomElementAmount>
